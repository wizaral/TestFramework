...
#undef CHECK_COMPARE
    ...
    template <class T, class U>
    static bool compare(const T &t, const U &u) {
        if constexpr (has_eq_op_v<T, U>) {
            ...
            constexpr auto has_any_comparison_operator = has_eq_op_v<T, U> || has_ne_op_v<T, U> || has_lt_op_v<T, U> || has_gt_op_v<T, U> || has_le_op_v<T, U> || has_ge_op_v<T, U>;
...
#define PRINT_CONTAINER(cont)                                                     \
    ...
    friend std::ostream &operator<<(std::ostream &os, const std::cont<T> &cont) { \
        return print_container(os, cont);                                         \
    }
